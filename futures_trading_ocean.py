...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df.ema5 > self.df.ema8) & (self.df.ema5 > self.df.ema10) & (self.df.ema8 > self.df.ema10) & (self.df['rsi'].between(50,60)) & (self.df.macd > 0), 1, 0)
        self.df['Sell'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df.ema5 < self.df.ema8) & (self.df.ema5 < self.df.ema10) & (self.df.ema8 < self.df.ema10) & (self.df['rsi'].between(40,50)) & (self.df.macd < 0), 1, 0)
        ...
        for check_balance in acc_balance:
            if check_balance['asset'] == "USDT":
                ...
                if df.Buy.iloc[-1]:
                    ...
                    file = open(file_path+ pair +'_buy_future_ocean.txt', 'w')
                    file.close()
                    ###########################################################################################################
                    #####################Read the previous sell text output and empty the file ###############################
                    with open(file_path+ pair +'_sell_future_ocean.txt', 'r') as f:
                        clean_sell_list = []
                        for sell_list in f.readlines():
                            clean_sell_list.append(sell_list.replace("\n", ""))
                    file = open(file_path+ pair +'_sell_future_ocean.txt', 'w')
                    file.close()
                    ##########################################################################################################
                    if pair not in clean_buy_list and float(open_position_check['entryPrice']) == 0:
                        ...
                        print(body)
                    with open(file_path+ pair +'_buy_future_ocean.txt', 'a+') as f:
                        f.write(str(pair) + '\n')
                    ...
                    if pair not in clean_sell_list and float(open_position_check['entryPrice']) == 0:
                        ...
                        for item in fees:
                            ...
                            requests.get(base_url)
                            print(body)
                    with open(file_path+ pair +'_sell_future_ocean.txt', 'a+') as f:
                        f.write(str(pair) + '\n')
...
while True:
    ...
    for coins in crypto_coins:
        try:
            ...
            total_coins = round(float(200/(float(current_price['price']))),1)
            print(total_coins)

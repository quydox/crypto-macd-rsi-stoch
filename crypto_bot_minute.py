...
def strategy(pair, qty, open_position=False):
    ...
    if df.Buy.iloc[-1]:
        ...
        file.close()
        ###########################################################################################################
        if (pair not in clean_sell_list and (df.Close.iloc[-1] <= int(clean_buy_price_list * .85))) or (pair not in clean_sell_list and df.Close.iloc[-1] > clean_buy_price_list):

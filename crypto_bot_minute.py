...
def strategy(pair, qty, open_position=False):
    ...
    if df.Buy.iloc[-1]:
        ...
        if (pair not in clean_sell_list and (df.Close.iloc[-1] <= clean_buy_price_list * float('.85'))) or (pair not in clean_sell_list and df.Close.iloc[-1] > clean_buy_price_list):

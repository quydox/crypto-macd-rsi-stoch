...
df = getminutedata('BTCUSDT', '1w', "60 day ago UTC")
# print(df)
...
# def strategy(pair, qty, open_position=False):
    # df = getminutedata(pair, '1w', "90 day ago UTC")
    # applytechnicals(df)
    # inst = Signals(df, 25)
    # inst.decide()
    ...
    # elif df.Sell.iloc[-1]:
        ...
        # if pair not in clean_sell_list:
            ...
            # body = pair,"SELL - 1 week timeframe " + str(df.Close.iloc[-1])
            # base_url = 'https://api.telegram.org/bot' + str(api_telegram) + '/sendMessage?chat_id=' + str(msg_id_telegram)+ '&text="{}"'.format(body)
            # requests.get(base_url)
            # print(body)
        # with open('/root/trading/'+ pair +'_sell_1wk.txt', 'a+') as f:
            # f.write(str(pair) + '\n')
# while True:

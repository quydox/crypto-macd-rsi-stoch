...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df['%K'] > self.df['%D']) & (self.df.ema5 > self.df.ema8) & (self.df.ema5 > self.df.ema10) & (self.df.ema8 > self.df.ema10) &  (self.df['rsi'].between(50,60)) & (self.df.macd > 0), 1, 0)
        ...
        for check_balance in acc_balance:
            if check_balance['asset'] == "BUSD":
                ...
                if df.Buy.iloc[-1] and float(open_position_check['entryPrice']) == 0:
                    ...
                    print(body)
                elif ((df.TPBUY1.iloc[-1] or df.TPBUY2.iloc[-1] or df.TPBOTH.iloc[-1]) and float(open_position_check['entryPrice']) != 0):
                    ...
                    for item in fees:
                        ...
                        requests.get(base_url)
                        print(body)
                elif ((df.TPSELL1.iloc[-1] or df.TPSELL2.iloc[-1] or df.TPBOTH.iloc[-1]) and float(open_position_check['entryPrice']) != 0):
                    order = client.futures_create_order(symbol=pair, side='BUY', type='MARKET', quantity=qty, leverage=3)
                    open_position = True
...
while True:
    ...
    for coins in crypto_coins:
        try:
            df = getminutedata(coins, '15m', "7 days ago SGT")

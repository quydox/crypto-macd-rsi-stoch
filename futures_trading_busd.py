...
class Signals:
    ...
    def decide(self):
        ...
        self.df['TPBUY3'] = np.where((self.df.trigger) & (self.df.uptrend.iloc[-1] < 1 ) & (self.df.downtrend.iloc[-1] < 1) & (self.df['%K'] < self.df['%D']), 1, 0)
        self.df['TPSELL3'] = np.where((self.df.trigger) & (self.df.uptrend.iloc[-1] < 1 ) & (self.df.downtrend.iloc[-1] < 1) & (self.df['%K'] > self.df['%D']), 1, 0)

...
class Signals:
    ...
    def decide(self):
        ...
        self.df['TPBUY1'] = np.where((self.df.trigger) & (self.df.ema5 < self.df.ema8) & (self.df.uptrend.iloc[-1]) & (self.df.rsi > 70), 1, 0)
        self.df['TPSELL1'] = np.where((self.df.trigger) & (self.df.ema5 > self.df.ema8) & (self.df.downtrend.iloc[-1]) & (self.df.rsi < 30), 1, 0)

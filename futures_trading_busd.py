        ...
        for check_balance in acc_balance:
            if check_balance['asset'] == "BUSD":
                ...
                if df.Buy.iloc[-1] and float(open_position_check['entryPrice']) == 0:
                    ...
                    print(body)
                elif ((df.TPBUY1.iloc[-1] or df.Sell.iloc[-1]) and float(open_position_check['entryPrice']) != 0):
                    ...
                    for item in fees:
                        ...
                        requests.get(base_url)
                        print(body)
                elif ((df.TPSELL1.iloc[-1] or df.Buy.iloc[-1]) and float(open_position_check['entryPrice']) != 0):
                    order = client.futures_create_order(symbol=pair, side='BUY', type='MARKET', quantity=qty, leverage=2)
                    open_position = True

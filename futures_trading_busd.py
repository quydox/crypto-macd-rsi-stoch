...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df.ema50 > self.df.ema100) & (self.df.ema50 > self.df.ema150) & (self.df.ema100 > self.df.ema150) & (self.df['rsi'].between(50,56) & (self.df.macd > 0)), 1, 0)
        self.df['Sell'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df.ema50 < self.df.ema100) & (self.df.ema50 < self.df.ema150) & (self.df.ema100 < self.df.ema150) & (self.df['rsi'].between(44,50)& (self.df.macd < 0)), 1, 0)
            ...
            if check_balance['asset'] == "BUSD":
                ...
                if df.Buy.iloc[-1]:
                    ...
                    with open(file_path+ pair +'_buy_future_busd.txt', 'a+') as f:
                        f.write(str(pair) + '\n')
                elif ((df.TPBUY1.iloc[-1] or df.TPBUY2.iloc[-1]) and df.uptrend.iloc[-1] and float(open_position_check['entryPrice']) != 0):
                    #####################Read the previous buy text output and empty the file ################################
                    with open(file_path+ pair +'_buy_future_busd.txt', 'r') as f:
                        clean_buy_list = []
                        for buy_list in f.readlines():
                    ...
                    with open(file_path+ pair +'_sell_future_busd.txt', 'a+') as f:
                        f.write(str(pair) + '\n')
                elif ((df.TPSELL1.iloc[-1] or df.TPSELL2.iloc[-1]) and df.downtrend.iloc[-1] and float(open_position_check['entryPrice']) != 0):
                    #####################Read the previous sell text output and empty the file ###############################
                    with open(file_path+ pair +'_sell_future_busd.txt', 'r') as f:
                        clean_sell_list = []
                        for sell_list in f.readlines():

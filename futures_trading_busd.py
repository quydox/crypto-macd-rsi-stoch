...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df.trigger) & (self.df['%K'] > self.df['%D']) & (self.df.ema5 > self.df.ema8) & (self.df.ema5 > self.df.ema10) & (self.df.ema8 > self.df.ema10), 1, 0)
        self.df['Sell'] = np.where((self.df.trigger) & (self.df['%K'] < self.df['%D']) & (self.df.ema5 < self.df.ema10) & (self.df.ema8 < self.df.ema10), 1, 0)

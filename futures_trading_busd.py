...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df['%K'] > self.df['%D']) & (self.df.ema5 > self.df.ema8) & (self.df.ema5 > self.df.ema10) & (self.df.ema8 > self.df.ema10) &  (self.df['rsi'].between(50,56)) & (self.df.macd > 0), 1, 0)
        self.df['Sell'] = np.where((self.df.trigger) & (self.df['%K'].between(20,80)) & (self.df['%D'].between(20,80)) & (self.df['%K'] < self.df['%D']) & (self.df.ema5 < self.df.ema8) & (self.df.ema5 < self.df.ema10) & (self.df.ema8 < self.df.ema10) & (self.df['rsi'].between(44,50)) & (self.df.macd < 0), 1, 0)
        ...
        self.df['TPBUY2'] = np.where((self.df.trigger) & (self.df.uptrend.iloc[-1]) & (self.df['rsi'] > 70 ), 1, 0)
        self.df['TPSELL2'] = np.where((self.df.trigger) & (self.df.downtrend.iloc[-1]) & (self.df['rsi'] < 30 ), 1, 0)

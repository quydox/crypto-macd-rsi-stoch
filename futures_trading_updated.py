...
class Signals:
    ...
    def decide(self):
        ...
        self.df['Buy'] = np.where((self.df['%K'] < 40) & (self.df['%D'] < 60) & (self.df.rsi > 50) & (self.df.macd > 0), 1, 0)
        self.df['Sell'] = np.where((self.df['%K'] > 60) & (self.df['%D'] > 60) & (self.df.rsi < 50) & (self.df.macd < 0), 1, 0)

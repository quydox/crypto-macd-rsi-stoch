...
df = getminutedata('BTCUSDT', '5m', "1 day ago UTC")
# print(df)
...
class Signals:
    ...
    def decide(self):
        ...
        self.df['overbought'] = np.where((self.df['%K'] > 70) & (self.df['%D'] > 70), 1, 0)
        self.df['oversold'] = np.where((self.df['%K'] < 30) & (self.df['%D'] < 30), 1, 0)
        ...
        self.df['Buy'] = np.where((self.df['%K'] < 30) & (self.df['%D'] < 30) & (self.df.rsi > 50) & (self.df.macd > 0), 1, 0)
        self.df['Sell'] = np.where((self.df['%K'] < 30) & (self.df['%D'] < 30) & (self.df.rsi < 50) & (self.df.macd < 0), 1, 0)
        ...
        # for check_balance in acc_balance:
            # if check_balance['asset'] == "BUSD":
                ...
                # elif df.Sell.iloc[-1]:
                    ...
                    # file.close()
                    # ###########################################################################################################
                    # if pair not in clean_sell_list or int(float(df.Close.iloc[-1])) <= int(float(open_position['entryPrice'])) * 0.999:

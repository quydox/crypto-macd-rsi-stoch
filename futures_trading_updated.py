...
def strategy(pair, qty, open_position=False):
    ...
    for open_position_check in active_position:
        #print(df)
        print(pair + " CLOSE PRICE " + str(df.Close.iloc[-1]) + "\n" + " ENTRY PRICE " + str(open_position_check['entryPrice']) + " MACD " + str(df.macd.iloc[-1]) + " RSI " + str(df.rsi.iloc[-1]) + " EMA " + str(df.ema.iloc[-1]) )
